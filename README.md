...
`claude-nim` is just `claude` CLI but pointed at NIM instead of Anthropic. It auto-wraps your session in tmux so it stays open when you close the terminal. You can switch models and expose to other devices without leaving the session.
...
### `/cc-switch <model>`
Switch to a different NIM model mid-conversation without losing context.
